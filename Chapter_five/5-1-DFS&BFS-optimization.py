...
def dfs(graph, start):
    ...
    start: 起始顶点
    返回:
    visited_order: 按DFS顺序访问的顶点列表
    """
    visited_set = set()  # 用于快速检查是否已访问，O(1)时间复杂度
    visited_order = []  # 存储访问顺序
    ...
    while stack:
        ...
        if vertex not in visited_set:
            visited_set.add(vertex)  # 标记为已访问
            visited_order.append(vertex)  # 记录访问顺序
            # 将未访问的邻居压入栈
